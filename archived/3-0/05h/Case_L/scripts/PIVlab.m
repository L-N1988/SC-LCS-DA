...
eps = max(1, min(floor(my / 10), floor(nx / 10)));
aoi = [center(1)-eps:center(1)+eps; center(2)-eps:center(2)+eps]; % area of interest
...
% fs is the frequency at which the acquired time-domain signal was sampled.

psdxs = zeros(lt/2, length(aoi)^2);
for i = 1:length(aoi(1, :))
    for j = 1:length(aoi(2, :))
        for k = 1:lt
            u_pri_t(k) = u_pri{k}(aoi(1, i), aoi(2, j));
        end
        x = u_pri_t; fs = 24;
        N = lt;
        xdft = fft(x);
        xdft = xdft(1:N/2); % low pass filter
        psdx = (1/(fs*N)) * abs(xdft).^2;
        psdx(2:end-1) = 2*psdx(2:end-1);
        psdxs(:, sub2ind([length(aoi(1, :)), length(aoi(2, :))], i, j)) = psdx;
    end
end

fs = 24; N = lt;
% See page 4: https://www.sjsu.edu/people/burford.furman/docs/me120/FFT_tutorial_NI.pdf
freq = 0:fs/N:fs/2-fs/N; freq = freq';

figure(1);
% subplot(4,1,1);
plot(freq,mean(psdx, 2));
grid on; set(gca, 'XScale', 'log'); set(gca, 'YScale', 'log');
set(xlabel("Frequency (Hz)", "FontSize", 14), 'Interpreter', 'latex'); 
set(ylabel("PSD (J/Hz)", "FontSize", 14), 'Interpreter', 'latex');
set(title("PSD Using FFT", FontSize=14), 'Interpreter', 'latex');
hold on;
xx = logspace(-4, 1, 100);
yy = xx.^(-1) / 10^5;
zz = xx.^(-5/3) / 10^5;
...
legend('PSD','$-1$', '$-\frac{5}{3}$', 'Interpreter', 'latex', fontsize=14)
hold off;

figure(2)
prePSD = mean(psdx, 2).*freq;
plot(freq, prePSD);
grid on; set(gca, 'XScale', 'log');
set(xlabel("Frequency (Hz)", "FontSize", 14), 'Interpreter', 'latex'); 
set(ylabel("PSD $\times$ Frequency (J)", "FontSize", 14), 'Interpreter', 'latex');
set(title("premultiplied PSD Using FFT", FontSize=14), 'Interpreter', 'latex');
...
xlabel("Frequency (Hz)")
ylabel("PSD (J/Hz)")
% myerr = max(max(psdx'-periodogram(x,rectwin(N),N,fs)))
...
% See page 2 and 3: https://www.sjsu.edu/people/burford.furman/docs/me120/FFT_tutorial_NI.pdf
S = S(1:(N/2)+1);
S(2:end-1) = 2*S(2:end-1);			% convert two-sided spectrum to single-sieded
