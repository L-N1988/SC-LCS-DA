...
[ii_c_old, jj_c_old] = deal(round(size(data_old.fs, 1)/2), round(size(data_old.fs, 2)/2));
[ii_c_new, jj_c_new] = deal(round(size(data_new.fs, 1)/2), round(size(data_new.fs, 2)/2));
...
plot(squeeze(data_old.fs(ii_c_old, jj_c_old, :)), squeeze(data_old.pxxs(ii_c_old, jj_c_old, :)), 'k', 'LineWidth', 1);
hold on;
plot(squeeze(data_new.fs(ii_c_new, jj_c_new, :)), squeeze(data_new.pxxs(ii_c_new, jj_c_new, :)), 'r', 'LineWidth', 0.5);
...
title(strcat('PSD of ', regexprep(substring, '\\', '-')));
